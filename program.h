...
#define VAR_NAME(x)             (#x)
#define DEBUG_VAR(x)			onDebugVariable(#x, x, __FUNCTION__)
#define DEBUG_FUNC				onDebugFunc(__FUNCTION__)
#define DEBUG_ERROR				onDebugError(__FUNCTION__, code, desc)
#define POP_ERROR(txt)          popErrorMsg(__FUNCTION__, txt);
...
#define SQL_ERROR(sql)                                                      \
    QMessageBox::critical(nullptr, QObject::tr("%1 Error").arg(__FUNCTION__),   \
                      sql.lastError().text())
